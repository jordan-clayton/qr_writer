...
impl ReedSolomon {
    ...
    // ... and so on.
    pub(crate) fn build_generator(&mut self, degree: usize) -> &GaloisPolynomial {
        if degree >= self.0.len() {
            let mut prev = self
                .0
                .last()
                .expect("Generator polynomials always start with 1.");
    ...
    // Per: https://www.thonky.com/qr-code-tutorial/error-correction-coding#step-8-generating-error-correction-codewords
    // Args:
    // - data -> the block of (only) codewords
    // - ec_bytes -> the number of ec_bytes in the block.
    //
    // Returns: the EC block as Vec<u8>
    pub(crate) fn encode(&mut self, data: &[u8], ec_bytes: usize) -> Vec<u8> {
